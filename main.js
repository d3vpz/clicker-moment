...
let upgrades = {
  ...
  cheese: {
    data: new Upgrade('cheese', 'Cheese', 'C H E Z', 100, 2),
  },
  bread: {
    data: new Upgrade('bread', 'Bread', 'wheat but more', 500, 4),
  },
  carrot: {
    data: new Upgrade('carrot', 'Carrot', 'More eyesight?', 1000, 8),
  },
};
...
function update_button_disp() {
  for (let key in upgrades) {
    let upgrade = upgrades[key].data;
    let button = document.getElementById(upgrade.id);
    if (button) {
      button.innerText = `${upgrade.name}: $${upgrade.cost}`; // Update button text based on upgrade properties
    } else {
      console.error(`Button with id ${upgrade.id} not found`);
    }
  }
}

function save() {
  // Serialize the upgrades object to a JSON string
  const serializedUpgrades = JSON.stringify(upgrades);

  // Save the serialized upgrades to localStorage
  localStorage.setItem('upgrades', serializedUpgrades);

  // Save the rest of the game data
...
function load() {
  // Retrieve the serialized upgrades from localStorage
  const serializedUpgrades = localStorage.getItem('upgrades');

  if (serializedUpgrades) {
    // Deserialize the upgrades object from the JSON string
    upgrades = JSON.parse(serializedUpgrades);
  }
